...
btnCerrarSesion.addEventListener("click", async () => {
  ...
  await fetch(`http://localhost:3000/user/logout`, requestOptions)
...
async function modificarNota(id, tituloViejo, textoViejo) {
  ...
  btnGuardar.onclick = async () => {
    ...
    await modificarNotaPut({ titulo: nuevoTitulo, texto: nuevoTexto }, id);
    location.reload();
...
async function eliminarNota(id) {
  ...
  await fetch(`http://localhost:3000/nota/${id}`, requestOptions)
    ...
    <div class="card h-100 shadow-sm border-0 bg-light">
      <div class="card-body d-flex flex-column">
        ...
        <div class="d-flex justify-content-end gap-2 mt-3">
          ...
          <button id="modificarNotita" onClick='modificarNota(${nota.id}, ${JSON.stringify(nota.titulo)}, ${JSON.stringify(nota.texto)})' data-bs-target="#modalModificar" data-bs-toggle="modalModificar" class="btn btn-outline-secondary mt-3 align-self-end"><i class="fa-solid fa-pen"></i></button>
        </div>
      </div>
    </div>
  `;
...
async function modificarNotaPut(datos, id) {
  const requestOptions = {
    method: "PUT",
    headers: HEADERS,
